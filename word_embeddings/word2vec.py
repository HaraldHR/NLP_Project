...
class Word2Vec(object):
    ...
    def get_context(self, i):
        ...
        for j in range(1, self.lws_size + 1):
            try:
                word_id = self.datapoints[i - j][
                    0
                ]  # can fetch previous words with self.datapoints
        ...
        for j in range(1, self.rws_size + 1):
            try:
                ...
                word_id = self.get_word_id(
                    word
                )  # initializes a new ID to the word, if it has not been seen.
        ...
        if os.path.isdir(file_or_dir):
            ...
            if self.tokenization is None:
                ...
                try:
                    ...
                    vocab, self.tokens, self.tokenized = self.tokenization.tokenize(
                        text
                    )
            ...
            for i, token in enumerate(self.tokenized):
                ...
                assert isinstance(
                    context, np.ndarray
                ), f"Context IDs are not in an numpy array. They have type {type(context)}"
                self.datapoints.append((focus_id, context))
                self.freq[
                    focus_id
                ] += (
                    1  # updates the frequency for the word in the unigram distribution.
                )
    ...
    def negative_sampling(self, number, focus, pos):
        ...
        samples = np.zeros(
            number
        )  # initializes the numpy array that contains all negative sample IDs.
    ...
    def train_vec(self):
        ...
        while self.current_epoch < self.epochs:
                ...
                neg_samples = np.array(
                    [
                        self.negative_sampling(
                            self.n_neg_samples, foc_word_id, pos_ex_id
                        )
                        for pos_ex_id in ctx_word_ids
                    ]
                ).reshape(
                    -1
                )  # shape: (C * n_neg_samples,)
            ...
            if self.lr_scheduling:
                print(
                    f"LR scheduling active: Epoch {self.current_epoch + 1} is finished using learning rate={self.lr}"
                )
            else:
                print(
                    f"Epoch {self.current_epoch + 1} finished. LR scheduling inactive."
                )
    ...
    def train(self):
        ...
        while self.current_epoch < self.epochs:
            for i in tqdm(range(len(self.datapoints))):
                ...
                foc_word_id = self.datapoints[i][
                    0
                ]  # the ID of the i'th token / datapoint in the text corpus.
                ctx_word_ids = self.datapoints[i][1]  # Of type numpy array.

                W = self.w_vector[
                    foc_word_id
                ]  # the specific focus word vector we need.

                grads_bit_pos = np.zeros(
                    (ctx_word_ids.shape[0], W.shape[0])
                )  # shape ctx_words x len of each vector
                grads_bit_neg = np.zeros(
                    (ctx_word_ids.shape[0] * self.n_neg_samples, W.shape[0])
                )

                for i, pos_ex_id in enumerate(ctx_word_ids):
                    W_tilde = self.w_tilde_vector[
                        pos_ex_id
                    ]  # the specific context vector we need.

                    # Calcs for the focus vector gradient.
                    bit_ctx = W_tilde * (
                        sigmoid(W_tilde.T @ W) - 1
                    )  # calculates pne part of the sum.
